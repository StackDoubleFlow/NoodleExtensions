...
MAKE_HOOK_MATCH(NoteCutter_Cut, &NoteCutter::Cut, void, NoteCutter* self, Saber* saber) {
  if (!Hooks::isNoodleHookEnabled()) return NoteCutter_Cut(self, saber);
  ...
  //    if (GeometryTools::ThreePointsToBox(p, saberBladeBottomPos, (bottomPos + topPos) * 0.5f, vector, halfExtents,
  //    orientation))
  //    {
  //        int num = Physics::OverlapBoxNonAlloc(vector, halfExtents, self->colliders, orientation,
  //        LayerMasks::_get_noteLayerMask()); if (num == 0)
  //        {
  //            return;
  //        }
  //        if (num == 1)
  //        {
  //            self->colliders[0]->get_gameObject()->GetComponent<CuttableBySaber*>()->Cut(saber, vector, orientation,
  //            saberBladeTopPos - topPos); return;
  //        }
  //        for (int i = 0; i < num; i++)
  //        {
  ...
  //            cuttableBySaberSortParams->distance = (topPos - position).sqrMagnitude() - component->get_radius() *
  //            component->get_radius(); cuttableBySaberSortParams->pos = position;
  //        }
  //        if (num == 2)
  //        {
  //            if (self->comparer->Compare(self->cuttableBySaberSortParams[0], self->cuttableBySaberSortParams[1]) > 0)
  //            {
  //                self->cuttableBySaberSortParams[0]->cuttableBySaber->Cut(saber, vector, orientation,
  //                saberBladeTopPos - topPos); self->cuttableBySaberSortParams[1]->cuttableBySaber->Cut(saber, vector,
  //                orientation, saberBladeTopPos - topPos); return;
  //            }
  //            self->cuttableBySaberSortParams[1]->cuttableBySaber->Cut(saber, vector, orientation, saberBladeTopPos -
  //            topPos); self->cuttableBySaberSortParams[0]->cuttableBySaber->Cut(saber, vector, orientation,
  //            saberBladeTopPos - topPos); return;
  //        }
  //        else
  //        {
  //
  //            System::Array::Sort((System::Array *) self->cuttableBySaberSortParams.operator Il2CppArray *(), 0, num,
  //            (System::Collections::IComparer*) self->comparer); for (int j = 0; j < num; j++)
  //            {
  //                self->cuttableBySaberSortParams[j]->cuttableBySaber->Cut(saber, vector, orientation,
  //                saberBladeTopPos - topPos);
  //            }
  //        }
  //    }
